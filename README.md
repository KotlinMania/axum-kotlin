...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`axum`](https://crates.io/crates/axum).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `axum` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Web framework on top of `tower` and `hyper`
...
Original work copyrighted by the upstream `axum` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
