# Claude Code Project Instructions — axum-kotlin

## Project Overview

This is **axum-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`axum`](https://crates.io/crates/axum).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/axum/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/axum>
